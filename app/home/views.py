...
class IndexHandler(HomeHandler):
    def get(self):
        ...
        if total >0:
            shownum = 20 # 每页列表显示的条数
            import math
            pagenum = int(math.ceil(total/shownum))
            if page<1 :
                self.redirect(self.request.path + "?page = %d&key = %s" % (1,t))
            if page>pagenum:
                self.redirect(self.request.path + "?page = %d&key = %s" % (pagenum,t))
...
class SearchHandler(HomeHandler):
    def get(self):
        ...
        offset = (page-1)*int(shownum)
        sql = "select a.id,a.title,a.img,a.info,t.name,a.addtime from art a left join tag t on a.tag = t.id where  a.title like :key or a.content like :key or a.info like :key limit :offset,:limit"
